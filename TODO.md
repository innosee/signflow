...
- [ ] **Audit-Log für Impersonation start/stop** — Helper in [src/lib/audit.ts](src/lib/audit.ts) ist da, aber `agency.actions.ts` (`impersonateCoach` / `stopImpersonating`) loggt noch nicht. Einbinden, sobald Real-Compliance-Anforderung greift.

## Tech-Debt / nice to have

- [ ] **"Agency" umbenennen** → `firma` / `traeger` (Owner ist die Firma, nicht die AfA-Agentur). Migration: `users.role`-Enum + `/agency/*`-Routen + Impersonation-Logik. Kein Zeitdruck, siehe Memory.
...
- [ ] **Agency-Monatsreport** (`/agency/reports`) — pro-Coach kumulierte UE, Fortschritt vs. Bewilligung, rein Query-Arbeit auf bestehenden Tabellen.
