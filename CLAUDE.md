...
10. **Abschluss:** Im Bridge-Modus **schließt die Bildungsträger-Freigabe aus Schritt 9 den Nachweis direkt ab** — `approveCourseReview` setzt `fes_status='completed'` und macht das finale PDF (HTML → Puppeteer) verfügbar. Es gibt **keinen separaten Coach-„Nachweis abschließen"-Schritt** (der coach-seitige `sealCourse`-Pfad ist derzeit toter Code). **Stand Launch: einfache elektronische Signatur** (kein Siegel). Das **FES-Siegel** (`src/lib/fes.ts` `sealWithFes`, gemockt → später `FES_MODE=live`) wird zum coach-seitigen Schritt, sobald live.
